        ...
        <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
            <div class="p-4 space-y-4">
                ...
                <div class="flex items-center justify-between gap-4">
                    <Link :href="`/kolektif/${props.agency.id}/data`"
                        class="px-4 py-2 bg-blue-600 text-sm text-white rounded hover:bg-blue-700 mb-3">
                    Tampilkan Semua Peserta {{ props.agency.name }}
                    </Link>

                    <a :href="`/export/${props.agency.id}/${props.tipe}`" class="text-blue-600 hover:underline">
                        Unduh Excel
                    </a>

                    <input v-model="search" @input="fetchData" placeholder="Cari Nama / NISN / NIK..."
                        ...
                        <tr v-for="(user, index) in props.users.data" :key="user.id || index"
                            ...
                            <td class="border px-2 py-1 flex justify-between">
                                ...
                                <span v-if="user.photo == null"
                                    class="px-1 py-0.5 bg-gradient-to-r from-purple-400 via-pink-500 to-red-500 text-white text-xs rounded-full border-2 border-transparent hover:border-white transition duration-300">
                                    Belum Foto
                                </span>
                            </td>
